...
import { BROKER_AUTH_STORAGE_KEY, brokerApi, publicApi } from "../api/client";
...
export function BrokerAuthProvider({ children }) {
  ...
  useEffect(() => {
    async function hydrateBrokerAuth() {
      const token = localStorage.getItem(BROKER_AUTH_STORAGE_KEY);
      if (!token) {
        setLoading(false);
        return;
      }
      ...
      try {
        const response = await brokerApi.get("/api/broker-auth/me");
  ...
  async function login(tokenId) {
    const response = await publicApi.post("/api/broker-auth/login", { tokenId });
  ...
  async function logout() {
    try {
      await brokerApi.post("/api/broker-auth/logout");
    } catch (error) {
